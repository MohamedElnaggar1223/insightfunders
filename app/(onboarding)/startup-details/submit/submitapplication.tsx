    ...
    return (
        <div className='flex flex-col gap-8'>
            <div className='flex gap-2 items-center justify-center'>
                <Checkbox className='rounded-[4px] border-white checked:!text-white' checked={checked} onCheckedChange={(value) => setChecked(value ? true : false)} id="terms" />
                <label
                    htmlFor="terms"
            ...
            <button onClick={handleSubmit} disabled={!checked} className='w-full bg-[#FF7A00] text-white font-bold rounded-[2px] py-5 text-sm px-4'>{isSubmitting ? <Loader2 stroke="#fff" className='animate-spin mx-auto' /> : 'Submit Application'}</button>
            {error && (
