    ...
    return (
        <section className='flex w-full flex-col gap-4 mt-12'>
            <div className='flex items-center justify-between gap-4'>
                <p className='font-bold dashboardtablehead   text-black text-xl'>Others</p>
                ...
                <TableBody>
                    ...
                    <TableRow key={Others.id}>
                        <TableCell className="px-1 border-2 text-center border-[#EAEAEA]    w-[100px]">
                            <ViewBtn document_link={Others.document_link!} type='legalDocuments' />
                        </TableCell>
                        <TableCell className="px-1 border-2 text-center border-[#EAEAEA]  chartcontenttext ">{Others.name}</TableCell>
                        <TableCell className="px-1 border-2 text-center border-[#EAEAEA]  chartcontenttext ">{formatDate(new Date(Others?.created_at!))}</TableCell>
                        <TableCell className="px-1 border-2 text-center border-[#EAEAEA]  chartcontenttext ">{formatDate(new Date(Others?.updated_at!))}</TableCell>
                        <TableCell className="flex items-center justify-center gap-3 p-6 text-center border-[#EAEAEA]   ">
                            <LegalDocumentsActionBtns document_link={Others.document_link!} legalDocumentsId={Others.id!} />
                        </TableCell>
                    </TableRow>
                    ))}
                </TableBody>
