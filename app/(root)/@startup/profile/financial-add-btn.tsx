...
const investorSchema = z
  .string({
    ...
    required_error: "Investor name is required",
  })
  .min(2, {
    message: "Investor name must be at least 2 characters long",
  });
...
export default function FinancialAddBtn({ user }: { user: UserType }) {
  ...
  const onSubmit = async (values: z.infer<typeof financialRoundSchema>) => {
    ...
    const response = await createFinancialRound({
      ...values,
      date: values.date.toISOString(),
      amount: values.amount.toString(),
      round: values.round as
        | "Pre-seed"
        | "Seed"
        | "Series A"
        | "Series B"
  ...
  return (
    <>
      <div
        ...
        className="w-24 cursor-pointer h-10 bg-[#FF7A00] gap-2 text-white text-sm font-semibold rounded-[8px] flex items-center justify-center"
      >
        <CirclePlus size={16} />
        Add
      </div>
        ...
        <DialogContent className="bg-[#1A1A1A] rounded-xl border-none space-y-4">
          ...
          <Form {...form}>
            <form
              ...
              className="flex flex-col items-center justify-start gap-8 max-h-[90vh] overflow-auto"
            >
                      ...
                      <FormControl>
                        <>
                          {field?.value?.map((investorField, index) => (
                            ...
                            >
                              <input
                                ...
                                onChange={(e) =>
                                  field.onChange(
                                    field.value.map((investorField, i) =>
                                      i === index
                                        ? e.target.value
                                        : investorField
                                    )
                          ...
                          <button
                            ...
                            className="flex items-center justify-center gap-2 text-white ml-auto bg-black rounded-[4px] px-4 py-2"
                          >
                            <CirclePlus size={16} />
                            Add Investor
                          </button>
                    ...
                    <FormItem className="relative flex flex-col gap-1 w-screen max-w-[384px]">
                      ...
                      <FormControl>
                        <div className="relative">
                          ...
                          <input
                            className="flex flex-1 pl-8 pr-4 placeholder:font-light py-5 rounded-[2px] outline-none w-full"
                            placeholder="Amount"
                            {...field}
                            onChange={(e) => {
              ...
              <DialogFooter>
                <Button
                  ...
                  className="bg-white rounded-[8px] w-24"
                >
                  Cancel
                </Button>
                <Button
                  type="submit"
                  className="text-white bg-black rounded-[8px] w-24"
                >
                  {isLoading ? (
