...
export default function FeaturesBatches() {
  ...
  return (
    ...
    <div
      ...
      className="h-[132px] relative max-h-[132px] overflow-scroll overflow-x-hidden hide-scrollbar"
    >
      ...
      <div
        style={{ willChange: "transform" }}
        className="flex h-[264px] relative min-h-[132px] overflow-scroll flex-col overflow-x-hidden hide-scrollbar w-full"
      >
        <AnimatePresence initial={false} mode="sync">
          <motion.div
            layoutId="container"
            className="flex flex-1 flex-col w-full min-h-[132px]"
          >
            {items.map((item, index) => (
              <motion.div
                key={index}
                className="rounded-[5px] secondDelay change-background py-2 px-4 transition-all text-black text-base relative flex-1 flex items-center"
              >
                <motion.p
                  transition={{ duration: 0.4 }}
                  key={item}
                  layoutId={item}
                  className={cn(index === items.length - 1 && "hidden")}
                >
                  {item}
                </motion.p>
